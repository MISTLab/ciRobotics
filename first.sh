...
read PROJECTNAME
echo $PROJECTNAME
docker service create --mode=global  --with-registry-auth --constraint node.role==worker --name $PROJECTNAME  --env="DISPLAY" --env QT_X11_NO_MITSHM=1   --mount type=bind,source=/tmp/.X11-unix,destination=/tmp/.X11-unix u16-vbw2:4567/root/$PROJECTNAME ping google.com
